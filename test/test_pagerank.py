...
class PageRank:
    ...
    def calculate_pagerank(self, iterations=500, damping_factor = 0.85) -> test_node:
        self.create_inlink()
        all_nodes = set(self.graph.keys())
        for target_nodes in self.graph.values():
            all_nodes.update(target_nodes)
        ...
        for iteration in range(iterations):
            ...
            for node in node_list:
                #print(f"node: {self.graph[node]} number_outlinks: '{len(self.graph[node])}" )
                
                print(f"{node} outlinks: {self.graph[node]} + {len(self.graph[node])}")
                sum_part = 0.0

...
class Crawler:
    def __init__(self):
        ...
        self.queue = deque([])
        with open("./focused_crawler/focused_queue.csv", 'r') as csvfile:
            reader = csv.reader(csvfile)
            for row in reader:
                self.queue.append(row[0])
        
        self.start_seed = self.queue[0]
        self.visited = set()
        self.visited.add(self.start_seed)
        self.num_nodes = 1
        
    def crawl(self):
        with open("focued_data.csv", 'a', newline='', encoding="utf-8") as csvfile:
            ...
            while (pages_count < limit and self.queue):
                ...
                try:
                    ...
                    current_node = focused_node.Node(current_seed)
                    ...
                    if 'links' in page_data:
                        for ext_url in page_data['links']:
                            title = ext_url['title']
                            ns = ext_url['ns']
                            current_title = focused_node.Node(title)
                            if ns == 0:
                    ...
                    row = [current_node.title] + [i.title for i in self.graph[current_node]] 
                    writer.writerow(row)
                    pages_count += 1
        ...
        with open("./focused_crawler/focused_queue.csv", 'a', newline='', encoding="utf-8") as queue:
